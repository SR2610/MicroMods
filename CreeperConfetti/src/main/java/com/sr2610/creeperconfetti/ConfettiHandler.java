...
public class ConfettiHandler {
	...
	private boolean willExplodeToConfetti() {
		...
		if (rand.nextInt(100) < ConfigHandler.GENERAL.ConfettiChance.get() || ConfigHandler.GENERAL.ConfettiChance.get()==0)
			return false;
		else
			return true;
	}
	...
	private void spawnParticles(EntityCreeper creeper) {
		Minecraft.getInstance().particles.addEffect(new ParticleFirework.Starter(creeper.getEntityWorld(), creeper.posX,
				creeper.posY+0.5F, creeper.posZ, 0, 0, 0, Minecraft.getInstance().particles, generateTag(creeper,false)));
		if(creeper.getPowered())
			Minecraft.getInstance().particles.addEffect(new ParticleFirework.Starter(creeper.getEntityWorld(), creeper.posX,
					creeper.posY+2.5F, creeper.posZ, 0, 0, 0, Minecraft.getInstance().particles, generateTag(creeper,true)));
	}
